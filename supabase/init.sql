...
CREATE TABLE IF NOT EXISTS public.one_time_purchases (
  ...
  CONSTRAINT one_time_purchases_credits_used_chk CHECK (credits_used >= 0 AND credits_used <= interview_count),
...
CREATE TABLE IF NOT EXISTS public.interview_billing_allocations (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT interview_billing_allocations_source_chk CHECK (source IN ('free_quota', 'subscription', 'admin_entitlement', 'purchased_credit')),
