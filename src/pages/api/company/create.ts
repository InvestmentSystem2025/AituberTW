...
export default async function handler(
  ...
  // 初始化 company 的履歷審查邀請免費額度（company 維度、永久累計；idempotent）
  try {
    await supa.from('company_resume_review_quota').upsert(
      {
        company_id: company.id,
        free_quota: 3,
        updated_at: new Date().toISOString(),
    ...
    // 不阻擋公司建立；DB guard / quota API 也會補建
  }

  // 自動把本人加入 company_members 並設 admin
  const { error: mErr } = await supa.from('company_members').insert({
    company_id: company.id,
    profile_id: me.id,
    company_role: 'admin',
  })
