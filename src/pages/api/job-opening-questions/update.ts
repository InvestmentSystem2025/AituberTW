...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { id, company_id, job_opening_id, question_bank_id, detail, sort_order, is_active } = req.body || {}
  ...
  if (typeof question_bank_id === 'string' || question_bank_id === null) payload.question_bank_id = question_bank_id
  if (d !== undefined) payload.detail = d
  if (typeof sort_order === 'number') payload.sort_order = sort_order
  if (typeof is_active === 'boolean') payload.is_active = is_active

  const { error } = await supa.from('job_opening_questions').update(payload).eq('id', id).eq('company_id', company_id)
