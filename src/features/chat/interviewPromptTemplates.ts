...
export const PERSONALITY_QUESTION_LIST: string[] = [
  // 外向 / 內向
  '在團隊聚會或公司活動中，你通常會主動認識新朋友、加入話題，還是比較習慣在一旁觀察、只跟熟悉的人聊天？請舉一個最近的例子說明。',
  '如果團隊需要有人在會議上對外簡報或主持，你通常會主動爭取、被指派時欣然接受，還是比較傾向讓其他同事去負責？為什麼？',
  // 盡責 / 隨性
  '當你接到一項期限比較緊的任務時，你通常會如何安排自己的工作步驟與時間？請描述一個你覺得自己做得不錯或特別有成就感的案例。',
  '如果專案中途出現意料之外的問題，導致原定進度會被延誤，你通常會怎麼處理？例如：和誰溝通、怎麼評估優先順序、會不會主動提出替代方案等。',
  // 細心 / 粗心
  '請分享一次你在工作或學校專案中，因為細心（或不夠細心）而對結果造成明顯影響的經驗，包含當時發生了什麼事、你如何發現、最後怎麼處理。',
  '在處理重複性高或需要長時間專注的任務（例如：資料整理、測試、文件撰寫）時，你通常如何避免出錯或遺漏細節？如果曾經出過錯，你後來做了什麼調整？'
]
...
2. **第二步（人格相關題目）**：在面試者完成自我介紹後，請先依序詢問「人格判斷用問題列表」中的所有問題，不要跳過或重複問題，用來了解面試者在「外向/內向、盡責/隨性、細心/粗心」等面向的傾向。
...
評分格式：[SCORE_START]{"questionId":"{questionId}","questionText":"{questionText}","answerText":"{answerText}","scores":{"評估項目key":本題加減的分數,"另一個評估項目key":本題加減的分數},"totalScore":本題加減分數的總和,"deductions":{"扣分制項目key":["扣分原因1"]},"additions":{"加分制項目key":["加分原因1"]},"deductions_detail":"為何扣了?分的判斷原因（必要）","additions_detail":"為何加了?分的判斷原因（必要）","aiFeedback":"你的回饋內容"}[SCORE_END]
其中：
...
**人格判斷輸出規則（只在整場面試「最後一次」回應時必須加入）：**
- 在最後一次回應的評分 JSON 中，請額外加入一個 "personality" 欄位，其內容為一個 JSON 物件，例如：
  "personality": { "extraversion": "偏外向 / 偏內向 / 介於中間，並說明依據", "conscientiousness": "非常盡責 / 普通 / 較隨性，並說明依據", "detail_attentiveness": "偏細心 / 偏粗心 / 介於中間，並說明依據", "summaryText": "用1-3句完整總結面試者整體性格特徵與工作風格" }
- 非最後一次回應時，可以省略 "personality" 欄位，或輸出為 null。
